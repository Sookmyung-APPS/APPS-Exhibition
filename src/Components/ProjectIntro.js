...
const array = [
    ...
    {   id: 8,
        name: "강단이",
        project: "눈송이 메이커",
        value: "◼️ 제작자 : 강단이\n\n🗺️ 작품 소개\n\n여러 물약과 재료들을 통해 독특한 눈송이를 만들어보세요!",
        printTitle: function(){
            return <div>{this.project}</div>;
        },    
        printValue: function() {
    ...
    {   id: 12,
        name: "이가을",
        project: "SNOW JUMP",
        value: "◼️ 제작자 : 이가을\n\n🗺️ 작품 소개\n눈송이가 아래로 떨어지지 않고 계속해서 올라가며 점수를 획득하는 점프 게임입니다."+ 
        "점프는 자동으로 되기 때문에 A, D 키로 캐릭터를 좌우로 움직여주면 됩니다. 선물 상자를 먹으면 점수가 더 많이 올라가지만, 눈송이가 싫어하는 과제를 먹으면 점수가 떨어집니다. \n",
        printTitle: function(){
            return <div>{this.project}</div>;
        },    
        printValue: function() {
    ...
    {   id: 13,
        ...
        value: "◼️ 제작자 : 장나리\n\n🗺️ 작품 소개\n 장애물에 걸리면 코인을 잃게 된다. 무사히 목적지에 도착할 수 있을지 ?!\n\n"+
        "🎮 플레이 방법\n화면의 점프버튼을 한번 클릭하면 눈송이가 한번 점프하고, 두 번 클릭하면 이단 점프를 한다.\n"+ 
        "코인을 먹으면 점수가 올라가고, 장애물에 걸리면 점수가 떨어진다.\n 점수가 0이 되거나 물에 빠지면 게임오버된다.",
        printTitle: function(){
            return <div>{this.project}</div>;
        },    
        printValue: function() {
