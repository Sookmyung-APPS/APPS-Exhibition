...
const project = [ "순간포착", "Delivery Run", "일력(dia+lendar)","LightAway","Sound Of Fairies","소프트웨어학부 소개 홈페이지","A LITTLE DRUID - 작은 마법사 : 잊혀진 기억을 찾아서", "눈송이 변신 물약",
"Terraforming (테라포밍)","소원을 들어주세요!","Devil","SNOW JUMP","눈송이가 길을 걸으며 코인을 얻는 게임.","2D 눈송이 게임","Rolling Ball","눈송이 공 굴리기"]
...
function ProjectIntro ( {name}) {
    ...
    return (
        <div >
            <h3 style={h3Style}><div>{project[index]}</div></h3>
            <div style={divStyle} key={array[index].name}>
